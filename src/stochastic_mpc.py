...
def solve_two_stage_lp(
    ...
    for s in range(S):
        for k in range(h):
            ...
            c[iF(s, k)] =  w[k] * (pi_e_sc[k, s] * alpha_sc[k, s] - pi_f_sc[k, s])
    ...
    # SoC dynamics per (s, k):  E_{s,k} - E_{s,k-1} + P_{s,k} - α F_{s,k} = (E0 if k==0 else 0)
    for s in range(S):
        for k in range(h):
            Arow += [row, row, row]
            Acol += [iE(s, k), iP(s, k), iF(s, k)]
            Aval += [1.0,      1.0,     -alpha_sc[k, s]]
            if k == 0:
                beq.append(E0)
            else:
    ...
    for s in range(S):
        for k in range(h):
            ...
            # (2a) SoC buffer on E_{k-1}  (use E0 for k==0)
            if k == 0:
            ...
            # (4) No sell-back:  P + F ≤ L_{s,k}
            add_row([(iP(s, k), 1.0), (iF(s, k), 1.0)], Lk)

            # Peak-demand epigraph:  -D_s - P + α F  ≤ -L_k
    ...
    x = res.x
    return float(x[iP(0, 0)]), float(x[iF(0, 0)]), 0
